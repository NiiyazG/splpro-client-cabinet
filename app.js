...
const defaults = {
  ...
  calculations: [
    { number: 'ST-26481', projectId: 'OBJ-0248', product: 'Насосная станция SPL', url: 'https://splpro.ru/selections/station', source: 'splpro.ru', createdBy: 'Нияз Гарипов', importedAt: '2026-08-18T10:24:00.000Z', status: 'Готов' },
    { number: 'AQ-26452', projectId: 'OBJ-0211', product: 'Насосы AQUASTRONG', url: 'https://aquastrong-select.ru/selection/series', source: 'aquastrong-select.ru', createdBy: 'Нияз Гарипов', importedAt: '2026-08-17T14:10:00.000Z', status: 'Согласован' },
    { number: 'ST-26398', projectId: '', product: 'Насосная станция SPL', url: 'https://splpro.ru/selections/station', source: 'splpro.ru', createdBy: 'Нияз Гарипов', importedAt: '2026-08-16T09:35:00.000Z', status: 'Новый' }
  ],
  selectionRequests: [
...
function normalizeState(value) {
  ...
  const calculations = normalizeCollection(merged.calculations, item => {
    ...
    const status = ['Новый', 'Готов', 'На проверке', 'Согласован', 'Импортирован'].includes(item.status) ? item.status : 'Импортирован';
    return { number, projectId, product, url: url.href, source: url.hostname.replace(/^www\./, ''), createdBy: safeText(item.createdBy, 'Текущий пользователь', 160), status, version: safeText(item.version, 'v1', 40), importedAt: safeDateTime(item.importedAt), decisionAt: safeDateTime(item.decisionAt) };
...
function renderCalculations() {
  ...
  byId('calculationsBody').innerHTML = items.map(item => {
    const trustedUrl = validatedCalculationUrl(item); const linked = Boolean(projectById(item.projectId)); const agreed = item.status === 'Согласован'; const importedAt = item.importedAt ? new Date(item.importedAt).toLocaleDateString('ru-RU') : 'Нет данных';
    const projectOptions = `<option value="">Не привязан</option>${state.projects.map(project => `<option value="${escapeHtml(project.id)}" ${project.id === item.projectId ? 'selected' : ''}>${escapeHtml(project.name)}</option>`).join('')}`;
    const decision = linked && !agreed ? `<button class="text-button" data-calculation-approve="${escapeHtml(item.number)}">Согласовать</button><button class="text-button" data-calculation-return="${escapeHtml(item.number)}">На доработку</button>` : '';
    const statusText = linked ? item.status : 'Не привязан'; const statusClass = !linked ? 'warning' : agreed ? 'success' : item.status === 'Готов' || item.status === 'Импортирован' ? 'neutral' : 'warning';
    const orderAction = linked ? `<button class="text-button" data-create-order="${escapeHtml(item.number)}" ${agreed ? '' : 'disabled title="Сначала согласуйте результат"'}>Создать заказ</button>` : '<span class="form-hint">Сначала привяжите к объекту</span>';
    return `<tr data-selection-row="${escapeHtml(item.number)}"><td data-label="Подбор"><div><strong>${trustedUrl ? `<a href="${escapeHtml(trustedUrl.href)}" target="_blank" rel="noreferrer">${escapeHtml(item.number)}</a>` : escapeHtml(item.number)}</strong></div></td><td data-label="Дата"><div>${escapeHtml(importedAt)}</div></td><td data-label="Оборудование"><div>${escapeHtml(item.product)}<br><small>${escapeHtml(item.source)}</small></div></td><td data-label="Выполнил"><div>${escapeHtml(item.createdBy)}</div></td><td data-label="Объект"><div class="selection-cell-controls"><select class="compact-select selection-project-select" data-selection-project="${escapeHtml(item.number)}">${projectOptions}</select><button class="text-button" data-bind-selection="${escapeHtml(item.number)}">${linked ? 'Изменить объект' : 'Привязать к объекту'}</button></div></td><td data-label="Версия / статус"><div>${escapeHtml(item.version)}<br><span class="tag ${statusClass}">${escapeHtml(statusText)}</span></div></td><td data-label="Действия"><div class="selection-cell-controls">${decision}${orderAction}</div></td></tr>`;
  }).join('') || '<tr><td colspan="7">Подборы не найдены.</td></tr>';
}
...
function populateFormOptions() {
  ...
  byId('orderCalculation').innerHTML = state.calculations.filter(item => item.status === 'Согласован' && projectById(item.projectId)).map(item => `<option value="${escapeHtml(item.number)}">${escapeHtml(item.number)} · ${escapeHtml(projectName(item.projectId))}</option>`).join('');
...
byId('calculationForm').addEventListener('submit', event => { event.preventDefault(); const number = safeId(byId('calculationNumber').value.trim()); const product = byId('calculationProduct').value; const url = validatedCalculationUrl({ product, url: byId('calculationUrl').value }); if (!number) return toast('Номер результата содержит недопустимые символы'); if (!url) return toast('Ссылка, путь и оборудование должны соответствовать доверенному конфигуратору'); const calculation = { number, projectId: byId('calculationProject').value, product, url: url.href, source: url.hostname.replace(/^www\./, ''), createdBy: 'Нияз Гарипов', status: 'Импортирован', version: 'v1', importedAt: new Date().toISOString() }; if (state.calculations.some(item => item.number === calculation.number)) return toast('Такой номер уже импортирован'); state.calculations.unshift(calculation); save(); closeDialog(byId('calculationDialog')); event.target.reset(); renderAll(); toast('Подбор добавлен в вашу историю'); });
byId('calculationSearch').addEventListener('input', renderCalculations);
byId('refreshMySelections').addEventListener('click', () => { const number = uid('WEB'); state.calculations.unshift({ number, projectId: '', product: 'Насосная станция SPL', url: `https://splpro.ru/selections/station?result=${encodeURIComponent(number)}`, source: 'splpro.ru', createdBy: 'Нияз Гарипов', status: 'Новый', version: 'v1', importedAt: new Date().toISOString(), decisionAt: '' }); save(); renderCalculations(); toast('Получен 1 новый подбор. Привяжите его к объекту'); });
byId('calculationsBody').addEventListener('click', event => { const bind = event.target.closest('[data-bind-selection]'); if (bind) { const calculation = state.calculations.find(item => item.number === bind.dataset.bindSelection); const select = event.target.closest('tr').querySelector('[data-selection-project]'); if (!calculation || !projectById(select.value)) return toast('Выберите объект из списка'); calculation.projectId = select.value; calculation.status = calculation.status === 'Новый' ? 'Готов' : calculation.status; save(); renderCalculations(); populateFormOptions(); toast('Подбор привязан к объекту'); return; } const approve = event.target.closest('[data-calculation-approve]'); const returned = event.target.closest('[data-calculation-return]'); if (approve || returned) { const calculation = state.calculations.find(item => item.number === (approve?.dataset.calculationApprove || returned?.dataset.calculationReturn)); if (!calculation || !projectById(calculation.projectId)) return toast('Сначала привяжите подбор к объекту'); calculation.status = approve ? 'Согласован' : 'На проверке'; calculation.decisionAt = new Date().toISOString(); save(); renderCalculations(); populateFormOptions(); toast(approve ? 'Версия результата согласована' : 'Результат возвращён на доработку'); return; } const button = event.target.closest('[data-create-order]'); if (!button || button.disabled) return; const calculation = state.calculations.find(item => item.number === button.dataset.createOrder && item.status === 'Согласован' && projectById(item.projectId)); if (!calculation) return toast('Заказ доступен только из привязанного и согласованного результата'); openDialog('orderDraftDialog'); byId('orderCalculation').value = calculation.number; });
...
byId('orderDraftForm').addEventListener('submit', event => { event.preventDefault(); const calculation = state.calculations.find(item => item.number === byId('orderCalculation').value && item.status === 'Согласован' && projectById(item.projectId)); if (!calculation) return toast('Заказ доступен только из привязанного и согласованного результата'); const id = uid('DRAFT'); const order = { id, projectId: calculation.projectId, product: calculation.product, amount: 0, ready: byId('orderDesiredDate').value.split('-').reverse().join('.'), paid: 0, paymentStatus: 'Не оплачено', supplyStatus: 'Не размещён', reserveUntil: '—', debt: 0, status: 'Черновик', tag: 'neutral', source: 'Кабинет → ожидает 1С', deliveryAddress: byId('orderDeliveryAddress').value.trim(), comment: byId('orderComment').value.trim(), actions: [{ id: uid('action'), title: 'Подтвердить реквизиты и договор', due: 'до отправки в 1С', done: false }, { id: uid('action'), title: 'Подтвердить адрес поставки', due: 'до отправки в 1С', done: false }] }; state.orders.unshift(order); state.selectedOrderId = id; save(); closeDialog(byId('orderDraftDialog')); event.target.reset(); renderAll(); goTo('orders'); toast('Черновик создан. Ожидает передачи в 1С'); });
